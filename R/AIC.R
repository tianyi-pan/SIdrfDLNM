...
#' @export
AIC.sidDLNM_fit <- function(object, verbose = FALSE, marginalAIC = FALSE) {
  ...
  ad.address <- mod.build$address.cppad


  H.LAML <- object$H.LAML
  H.LAML.par <- H.LAML[-1,-1]
  cprior <- 1e-3 # following mgcv
  Vrho <- Matrix::solve(H.LAML.par + diag(rep(cprior, nrow(H.LAML.par))))
  if(marginalAIC == TRUE) {
    if(all.equal(H.LAML, diag(nrow(H.LAML)))) {
      warning("Hessian of LAML is not calculated. Cannot compute marginal AIC; use conditional AIC instead.")
    }
  }
  AIC <- ConditionalAICsidDLNM(mod.address, ad.address, Vrho, verbose, marginalAIC)
  if(verbose) cat("finished AIC calculation ... \n")
