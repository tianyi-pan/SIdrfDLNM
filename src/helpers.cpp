...
// [[Rcpp::export]]
double SurfaceEval(double E, double Ecen, int l, Eigen::VectorXd alphaf,
                   Eigen::VectorXd knotsf, Eigen::MatrixXd Z,
                   Eigen::MatrixXd Blag) {
  Eigen::VectorXd Bf_tmp(alphaf.size());
  Bf_tmp.setZero();
  int kl = Blag.rows();
  int kx = knotsf.size() - 4 - 1;

  Eigen::VectorXd bx = BsplinevecCon(E, knotsf, 4, Z) - BsplinevecCon(Ecen, knotsf, 4, Z);
  for (int ii = 0; ii < kx; ii++) {
    Bf_tmp.segment(ii*kl, kl) = bx(ii) * Blag.col(l);
  }

  return Bf_tmp.dot(alphaf);
}

// [[Rcpp::export]]
Eigen::MatrixXd SurfaceCI(Eigen::MatrixXd gridEl, Eigen::MatrixXd alpha_f_sample,
                          double Ecen,
                          Eigen::VectorXd knotsf, Eigen::MatrixXd Z,
                          Eigen::MatrixXd Blag) {
  int n = gridEl.rows();
  int R = alpha_f_sample.rows();
  Eigen::MatrixXd out(n,R);
  for (int j = 0; j < R; ++j) {
    for  (int i = 0; i < n; ++i) {
      out(i,j) = SurfaceEval(gridEl(i,0), Ecen, int (gridEl(i,1)), alpha_f_sample.row(j), knotsf, Z, Blag);
    }
  }
  return out;
}
